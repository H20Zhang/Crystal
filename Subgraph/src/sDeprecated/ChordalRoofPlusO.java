 ...
 * @author zhanghao
 */
public class ChordalRoofPlusO extends Configured implements Tool {
    private static Logger log = Logger.getLogger(ChordalRoofPlusO.class);
    ...
    public static class ChordalRoofPlusOMapper extends Mapper<TwoNodeKey, IntDegreeArray, IntArray, IntArray> {
	...
	protected void map(TwoNodeKey key, IntDegreeArray value, Context context)
		...
		while (iterator.hasNext()) {
			...
			if (theValue.size() >= 3) {
			    long counter = (theValue.size() * (theValue.size() - 1) * (theValue.size() - 2) / 6);
		...
		while (iterator.hasNext()) {
			...
			if (theValue.size() >= 3) {
			    ...
			    context.getCounter("test", "list_size").increment(theValue.size());
	...
	job.setMapperClass(ChordalRoofPlusOMapper.class);
	...
	for (int i = 0; i < p; i++) {
	    ...
	    ToolRunner.run(conf, new ChordalRoofPlusO(), args);
	}

	endTime = System.currentTimeMillis();
	log.info("[chordal roof plus] Time elapsed: " + (endTime - startTime) / 1000 + "s");
    }
