...
public class ChordalSquarePlusO extends Configured implements Tool {

    private static Logger log = Logger.getLogger(ChordalSquarePlusO.class);
	...
	protected void map(TwoNodeKey key, IntDegreeArray value, Context context)
		...
		context.getCounter("test", "pattern_size")
			.increment((long) value.size * (value.size - 1) * (value.size - 2) / 6);
	    }
	}
    }
	...
	ToolRunner.run(conf, new ChordalSquarePlusO(), args);
