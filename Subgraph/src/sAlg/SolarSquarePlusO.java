...
public class SolarSquarePlusO extends Configured implements Tool {
	...
	public static class SolarSquarePlusOMapper extends Mapper<TwoNodeKey, IntDegreeArray, IntArray, TwoIntArray> {
		...
		protected void map(TwoNodeKey key, IntDegreeArray value,
						   Mapper<TwoNodeKey, IntDegreeArray, IntArray, TwoIntArray>.Context context)
			...
			// left node
			{
				...
				while (iterator.hasNext()) {
					...
					if (theKey != key.getNode1() && theKey != key.getNode2()) {
						...
						int list_size_1 = map.get(key.node2).size();
						...
						outputValue.nodeArray1 = map.get(key.node2).toArray();
						...
						context.getCounter("test", "graph2")
								.increment((long) (list_size_1 - 3) * (list_size_2 * (list_size_2 - 1) / 2));
					}
				}
			}
			...
			// right node
			{
				...
				while (iterator.hasNext()) {
					...
					if (theKey != key.getNode1() && theKey != key.getNode2()) {
						...
						int list_size_1 = map.get(key.node1).size();
						int list_size_2 = theValue.size();
						...
						outputKey.setNode(2, key.getNode2());
						TwoIntArray outputValue = new TwoIntArray(list_size_1, list_size_2);
						outputValue.nodeArray1 = map.get(key.node1).toArray();
						outputValue.nodeArray2 = theValue.toArray();
						context.write(outputKey, outputValue);
